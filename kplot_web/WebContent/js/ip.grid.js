...
(function ($) {
    ...
    $.fn.ip_Grid = function (options) {
        ...
        if (options.onSize != null) { ip_GridProps[options.id].callbacks.onSize = options.onSize; }
    ...
    $.fn.ip_ColumnSelector = function (options) {
        ...
        if (!options.show) {
            ...
            //var colv = $('#' + GridID + '_q2_container th.ip_grid_columnSelectorCellCorner div.ip_grid_cell_outerContent');
    ...
    $.fn.ip_gridProperties = function (options) {
        ...
        var options = $.extend({
            ...
            //colSymbols: ip_ColumnSymbols(100),
            userDefineColumns : [],
            resizing: false,
            mouseButton: 0,
            shiftKey: false,
...
function ip_CreateGridQuadCell(options, tableColumnsArray) {
    ...
    if (options.cellType == 'RowSelector') {
        ...
        TableColumns = '<th cellType="ColSelector" class="ip_grid_columnSelectorCell q' + Quad + selected + hide + '" id="' + options.id + '_columnSelectorCell_' + col + '"  quadrant="' + options.Quad + '" row="-1" col="' + col + '"  style="' +
                                                                                                                                                           ...
                                                                                                                                                           '">' +
                               hideIcon +
                               '<div id="ht_'+col+'" class="ip_grid_cell_outerContent" style="position:relative;' +
                                                                                (!options.showColSelector ? 'display:none;' : '') +
                                                                                '" >' +
                                                                                getColumnDisplayName(options,col)
...
function getColumnDisplayName( options , col ){
	if( ip_GridProps[options.GridID].userDefineColumns != null && ip_GridProps[options.GridID].userDefineColumns.length > col &&
			ip_GridProps[options.GridID].userDefineColumns[col].length > 0 ){
		return ip_GridProps[options.GridID].userDefineColumns[col];
	}
	return ('V' + col + '');
}
...
function ip_ResizeGrid(GridID, newWidth, raiseEvent) {
	...
	if( ip_GridProps[GridID].callbacks.onSize != null ){
		ip_GridProps[GridID].callbacks.onSize( newWidth );
	}
	//debug.log( "grid width : " + newWidth );
    var Error = '';
    var FirstQuadTableWidth = parseInt($('#' + GridID + '_q1_table').outerWidth());
