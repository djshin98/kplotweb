...
function SheetX(type,datasource,target,jsonStrData){
	...
	this.tip = {offsetx:-70,offsety:0,format:".2f"};
	this.columns = null;
	...
	this.removeColumn = function(index){
		var fname = this.data.fields[index].name;
		this.data.fields = this.data.fields.filter(function(d,i){return (i==index)?false:true; });
	};
	this.removeRow = function(index){
		this.data.rows = this.data.rows.filter(function(d,i){return (i==index)?false:true; });
	};
	this.selectRows = function(list){
		this.data.rows = this.data.rows.filter(function(d,i){ return (list.indexOf(i)>=0)?true:false;});
	}
	this.copyData = function(data){
		this.data.fields = data.fields.slice();
		this.data.rows = data.rows.slice();
	}
	...
	this.renameField = function(index,name){
		var oldName = this.data.fields[index].name;
		this.data.fields[index].name = name;
		this.data.rows.forEach(function(d,i){
			d[name] = d[oldName];
			delete d[oldName];
		});
	};
	this.createWindow = function(id){
		var ids=kplot.panel.create(id);
	...
	this.color = function(name){
		if( typeof name == "number" ){
			return kplot.colors[name%kplot.colors.length];
		}else if( name && this.data.fields ){
...
var kplot = {
	...
	extend : function( option ){
		if( kplot.getCurrentSheet() ){
			var options = {
				bid : kplot.graphBodyId,
				originIndex : 0,
				barList : kplot.getCurrentSheet().data.fields.map(function(d,i){return (i==0)?-1:i; }).filter(function(d,i){ return (d>=0)?true:false; }),
				lineList : kplot.getCurrentSheet().data.fields.map(function(d,i){return (i==(kplot.getCurrentSheet().data.fields.length-1))?i:-1; }).filter(function(d,i){ return (d>=0)?true:false; }),
				xIndex : 1,
				yIndex : 2
			};
			if(option != undefined){
	...
	ui : {
		menu : {
			select : {
				...
				editor : function(graphParentId){
					if( kplot.getCurrentSheet() ){
						kplot.panel.createEditor(graphParentId);
					}
				},
				graph : {
					bar : {
						vertical : function(option) {
							var options = kplot.extend(option);
							var li = new LegendInfo();
							li.align("out").align("bottom");
							kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).columns(options.barList).legend(li).verticalBar();
						},
						horizontal : function(option) {
							var options = kplot.extend(option);
							var li = new LegendInfo();
							li.align("out").align("bottom");
							kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).columns(options.barList).legend(li).horizontalBar();
						},
						stack : function(option) {
							var options = kplot.extend(option);
							var li = new LegendInfo();
							li.align("out").align("bottom");
							kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).columns(options.barList).legend(li).verticalStack();
						},
						stackHorz : function(option) {
							var options = kplot.extend(option);
							var li = new LegendInfo();
							li.align("out").align("bottom");
							kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).columns(options.barList).legend(li).horizontalStack();
						},
						ratio : function(option) {
							var options = kplot.extend(option);
							var li = new LegendInfo();
							li.align("out").align("bottom");
							kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).columns(options.barList).legend(li).ratio();
						}
					},
					pie : {
						pie : function(option) {
							...
							var li = new LegendInfo();
							li.align("out").align("bottom");
							kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).columns(options.barList).legend(li).pie();
						},
						donut : function(option) {
							var options = kplot.extend(option);
							var li = new LegendInfo();
							li.align("out").align("bottom");
							kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).columns(options.barList).legend(li).pie();
						},
						band : function(option) {
							var options = kplot.extend(option);
							var li = new LegendInfo();
							li.align("out").align("bottom");
							kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).columns(options.barList).legend(li).pie();
					...
					line : function(option){
						...
						var li = new LegendInfo();
						li.align("out").align("bottom");
						kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).columns(options.lineList).legend(li).line();
					},
					scatter : function(option){
						var options = kplot.extend(option);
						var li = new LegendInfo();
						kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).x(options.xIndex).y(options.yIndex).legend(li).scatter();
					},
					complex : function(option){
						var options = kplot.extend(option);
						var li = new LegendInfo();
						li.align("out").align("bottom");
						kplot.getCurrentSheet().createWindow(options.bid).origin(options.originIndex).bars(options.barList).lines(options.lineList).legend(li).leftAxisText("Bar").rightAxisText("Line").complex();
					}
				},
	...
	panel : {
		...
		graph : {
			redraw : function(ev,lobiPanel){
				...
				var obj = kplot.graph.findGraphWindow( $svg.attr('id') );
				if( obj ){obj.redraw();}
			}
		},
		editor : {
			redraw : function(ev,lobiPanel){
				var $svg = $(lobiPanel.$el.find('textarea'));
				var owidth = $svg.parent().width();
				var oheight = $svg.parent().height();
				$svg.css('height',oheight-45);
			}
		},
		createEditor : function(graphParentId){
			var id = "kplot-editor";
			var panel = $('<div class="panel panel-default" style="left:20px;top:100px;"><div class="panel-heading"><div class="panel-title">'
					+'<h4 id="'+id+'-title" ></h4></div></div><div class="panel-body" style="padding:0px">'
					//+'<div class="media"><div class="media-left"></div>'
					//+'<div class="media-body">'
					+'<textarea id="kplot-editor" style="margin: 0px;width: 100%;height: 310px;resize: none;"></textarea>'
					+'<button type="button" style="margin-left:15px" class="btn btn-danger" onclick="kplot.eval( $(\'#'+'kplot-editor'+'\').val() )">Execute</button></div>'
					
					//+'</div>'
					+'</div></div>');
			panel.css("width","700px");
			panel.css("height","400px");
			$( graphParentId ).append(panel);
			panel.lobiPanel("unpin");
			
			panel.on('onFullScreen.lobiPanel', kplot.panel.editor.redraw);
			panel.on('onSmallSize.lobiPanel', kplot.panel.editor.redraw);
			panel.on('onPin.lobiPanel', kplot.panel.editor.redraw);
			panel.on('onUnpin.lobiPanel', kplot.panel.editor.redraw);
			panel.on('onReload.lobiPanel', kplot.panel.editor.redraw);
			panel.on('resizeStop.lobiPanel', kplot.panel.editor.redraw);
			
		},
		create : function(graphParentId){
			var ids = [];
			...
			panel.on('onFullScreen.lobiPanel', kplot.panel.graph.redraw);
			panel.on('onSmallSize.lobiPanel', kplot.panel.graph.redraw);
			panel.on('onPin.lobiPanel', kplot.panel.graph.redraw);
			panel.on('onUnpin.lobiPanel', kplot.panel.graph.redraw);
			panel.on('onReload.lobiPanel', kplot.panel.graph.redraw);
			panel.on('resizeStop.lobiPanel', kplot.panel.graph.redraw);
	...
	GraphWindow : function(id,sheetx){
		...
		this._guideline = {x:false,y:false};
		this.guideline = function(x,y){
			this._guideline.x = x;
			this._guideline.y = y;
			return this;
		};
		this.graphType = function(){
			if( this.drawfn == this.pie ){
				return GraphType.PIE;
			}
			return GraphType.BAR;
		}
		this.axisIndex = {x:-1,y:-1};
		this.x = function(xindex){
			this.axisIndex.x = xindex;
			return this;
		};
		this.y = function(yindex){
			this.axisIndex.y = yindex;
			return this;
		};
		...
		this.drawLegend = function(svg,keys,rectInfo,reverse){
			...
			//var legendNames=this.legendNames;
			//keys = keys.map(function(d,i){if(legendNames&&legendNames.length>i)return legendNames[i]; return d;});
...
//			var originValues = this.sheetx.data.rows.map(function(d,i){
//				return d[originName];
//			});
		...
		this.getRectInfo = function(vertical,complex){
			...
			if(this.graphType() == GraphType.PIE){
				keys = data.map(function(d,i){return d[originName];});
			}
				
			var ix = this.sheetx.getMinMax( data , this.columnKeys() );
			...
			if( this.legendInfo.style.indexOf("out")>=0){
				...
				if( complex ){
					...
					maxfieldWidth = keys.reduce(function(prev,curr){ 
			...
			if( vertical ){
				margin.left = Math.max( d3.format(".4f")(" "+ix.max).width() ,d3.format(".4f")(" "+ix.min).width() ) + 15;
			}else{
				margin.left = maxWidth + 20;
			}
		...
		this.drawXAxis = function(svg,xAxis,rectInfo){
			var xaxis = svg.append("g")
		      .attr("class", "x axis")
		      .attr("transform", "translate(0," + rectInfo.height + ")").call(xAxis);
		...
		this.drawGuideLine = function(svg,x,y,rectInfo){
			if(this._guideline.x){
			      svg.append("g")
			      .attr("class", "grid")
			      .attr("transform", "translate(0," + rectInfo.height + ")")
			      .call(d3.svg.axis().scale(x).orient("bottom").tickSize(-rectInfo.height, 0, 0).tickFormat(""));
		    }
			if(this._guideline.y){
				svg.append("g")         
		        .attr("class", "grid")
		        .call(d3.svg.axis().scale(y).orient("left").tickSize(-rectInfo.width, 0, 0).tickFormat("") );
			}
		}
		...
		this.verticalBar = function(){
			...
			var yaxis = this.drawYAxis(svg,data,keys,rectInfo,"left");
			/*
		    ...
		    //  .text("Population");
			*/
			this.drawGuideLine(svg,x0,yaxis.y,rectInfo);
			...
			var tipfmt = this.sheetx.tip.format;
			var tip = d3.tip()
			  .attr('class', 'd3-tip')
			  .offset([this.sheetx.tip.offsetx,this.sheetx.tip.offsety])
			  .html(function(d,i) {
			    return "<strong>"+d.name+":</strong> <span style='color:yellow'>" + d3.format(tipfmt)(d.value) + "</span>";
			  });
			...
			state.selectAll("rect").data( function(d) { return d.values; } ).enter().append("rect").attr("width", x1.rangeBand())
		      ...
		      .attr("height", function(d) { 
		    	  return rectInfo.height - yaxis.y(d.value); })
		  ...
		  if( this.isShowAverage() ){
			  ...
			  .offset([this.sheetx.tip.offsetx,this.sheetx.tip.offsety])
			  .html(function(d) {
			    return "<strong>"+d.name+" 평균 :</strong> <span style='color:yellow'>" + d3.format(tipfmt)(d.value) + "</span>";
			  });
	    	...
	    	g.selectAll(".cir").data(avgData).enter().append("circle").attr("class","cir")
	    	 ...
	    	 .attr("cy",function(d,i){
	    		// console.log( y(d.value) );
	    		 return yaxis.y(d.value); }).call(atip)
		...
		this.horizontalBar = function(){
			...
			this.drawGuideLine(svg,x,y0,rectInfo);
		...
		this.verticalStack = function(){
			...
			var xaxis = this.drawXAxis(svg,xAxis,rectInfo);
			  svg.append("g")
		      	.attr("class", "axis y")
		      	.call(yAxis);
			  this.drawGuideLine(svg,x,y,rectInfo);
			  var tipfmt = this.sheetx.tip.format;
			var tip = d3.tip()
			  .attr('class', 'd3-tip')
			  .offset([this.sheetx.tip.offsetx,this.sheetx.tip.offsety])
			  .html(function(d,i) {
			    return "<strong>"+d.x+":</strong> <span style='color:yellow'>" + d3.format(tipfmt)(d.y) + "</span>";
			  });
		...
		this.pie = function(){
			...
			var rectInfo = this.getRectInfo(true,false);
			var originName = this.originName();
			var radius = Math.min(rectInfo.width, rectInfo.height) / 2;
			var keys = this.columnKeys();
			...
			var tip = d3.tip()
			  ...
			  .offset([this.sheetx.tip.offsetx,this.sheetx.tip.offsety])
			  .html(function(d) {
			    return "<strong>"+d.data[originName]+":</strong> <span style='color:yellow'>" + d3.format(tipfmt)(d.value) + "</span>";
			  });
		...
		this.line = function(){
			...
			/*svg.append("g").attr("class", "y axis")
		      ...
		      .style("text-anchor", "end");*/
			var yaxis = this.drawYAxis(svg,data,keys,rectInfo,"left");
			this.drawGuideLine(svg,x,yaxis.y,rectInfo);
		    ...
		    keys.map(function(c){return data.map(function(d){return { cname:c,name:d[originName],value:d[c]};});})
		      .forEach(function(cold,a){
		    	 var g = svg.append("g");
		    	 var color = sheetx.color(cold[0].cname);
		    	 ...
		    	 .attr("cy",function(d,i){ return yaxis.y(d.value); }).call(tip)
		    	 .on("mouseover",function(o){
		    		 d3.select(this).transition().duration(500).ease("elastic").attr("r",9);
		    		 tip.show(o);
		    	 })
		...
		this.bar = function(){
			...
			this.drawGuideLine(svg,x0,y,rectInfo);
