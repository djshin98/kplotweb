...
var RequestLogin = function(){
	...
	this.init = function(){
		...
		this.prop.requestObject.userPassword='';
...
//등록
/*******************************************************************************************************************************************************************************
Mobile HSE , 2015.09 RequestAccount 등록
*******************************************************************************************************************************************************************************/
var RequestAccount = function(){
	this.prop = {
		requestObjectName : 'AppRequestAccount',
		requestObject :{ 
			requestKey : '',			//요청 키
			userId : '',				//아이디
			userName : '',			//성명
	...
	//RequestAccount 오브젝트 초기화
	this.init = function(){
		this.prop.requestObject.requestKey=(new IDGenerator()).generate();
		this.prop.requestObject.userId='';
		this.prop.requestObject.userName='';
		this.prop.requestObject.userEmail='';
		this.prop.requestObject.userPassword='';
		this.prop.requestObject.language='';

    };
    //RequestAccount 오브젝트를 json string로 변환
    this.toString = function(){
    	return JSON.stringify( this.prop );
    };
    this.request = function( onSuccessResponse , onFailResponse ){
    	//RequestAccount request Message를 서버에 요청 하고, ResponseAccount Message가 recevedData로 리턴받는다.
    	M.net.http.send({
			server:	'mhse',
			path:	'/api/msp/mhse/message',
		    method:	'POST',
		    ...
		    onSuccess:	function(receivedData, setting) {
		    	...
		    	if( receivedData.result == 'OK' ){
		    		...
		    		if( onFailResponse == undefined || onFailResponse == null ){
		    			alert( "Fail : " + receivedData.resultMessage );
		    		}else{ //RequestAccount 요청이 서버에 전송되었고, 처리중 실패(처리 부적합)로 판단하여 리턴 
		    			onFailResponse(receivedData, setting);
		    		}
		    	}
		    },
		    "onError":	function(errorCode, errorMessage, setting) { //RequestAccount 요청이 서버와 연결중 네트워크 또는 기타 오류...
		        M.tool.log( "onError", errorCode, errorMessage );
		    	alert( "onError, " + errorCode + ", " + errorMessage );
		    }
		});
...
//로그아웃
/*******************************************************************************************************************************************************************************
Mobile HSE , 2015.09 RequestLogout 로그아웃
*******************************************************************************************************************************************************************************/
var RequestLogout = function(){
	this.prop = {
		requestObjectName : 'AppRequestLogout',
		requestObject :{ 
			requestKey : '',			//요청 키
			language : '',			//모바일 사용자가 선택한 언어(한국어="ko",영어="en")
	...
	//RequestLogout 오브젝트 초기화
	this.init = function(){
		this.prop.requestObject.requestKey=(new IDGenerator()).generate();
		this.prop.requestObject.language='';

    };
    //RequestLogout 오브젝트를 json string로 변환
    this.toString = function(){
    	return JSON.stringify( this.prop );
    };
    this.request = function( onSuccessResponse , onFailResponse ){
    	//RequestLogout request Message를 서버에 요청 하고, ResponseLogout Message가 recevedData로 리턴받는다.
    	M.net.http.send({
			server:	'mhse',
			path:	'/api/msp/mhse/message',
		    method:	'POST',
		    ...
		    onSuccess:	function(receivedData, setting) {
		    	...
		    	if( receivedData.result == 'OK' ){
		    		...
		    		if( onFailResponse == undefined || onFailResponse == null ){
		    			alert( "Fail : " + receivedData.resultMessage );
		    		}else{ //RequestLogout 요청이 서버에 전송되었고, 처리중 실패(처리 부적합)로 판단하여 리턴 
		    			onFailResponse(receivedData, setting);
		    		}
		    	}
		    },
		    "onError":	function(errorCode, errorMessage, setting) { //RequestLogout 요청이 서버와 연결중 네트워크 또는 기타 오류...
		        M.tool.log( "onError", errorCode, errorMessage );
		    	alert( "onError, " + errorCode + ", " + errorMessage );
		    }
		});
